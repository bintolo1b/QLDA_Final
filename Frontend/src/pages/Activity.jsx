...
function Activity() {
    ...
    const [enableActivityFilter, setEnableActivityFilter] = useState(false)
    ...
    const activity_topside_card_container = {
        padding: "14px 25px",
        display: "flex",
    ...
    const activityTopSideAndStatusContainter = {
        position: 'sticky',
    ...
    const cards = [];
    for (let i = 0; i < 20; i++) {
        cards.push(<ActivityCard key={i} link={i%2==0?'/activities/chat':'/activities/cpnb'} onClick={() => setActivatedCard(i)}  isEnabled = {i===activatedCard?true:false}/>);
    }
    ...
    return (
        <Box sx={container}>
            <Box sx={activityCardContainer}>
                <Box sx={activityTopSideAndStatusContainter}>
                    <Box sx={activity_topside_card_container}>
                        <Box style={activityWord}>Hoạt động</Box>
                        <div>
                            <div 
                            ...
                            <SearchIcon onClick={()=>setEnableActivityFilter(!enableActivityFilter)} sx={lookUpIcon}/>      
                        </div>
                    </Box>

                    <Box sx={{display: 'flex', justifyContent: 'center'}}>
                        {enableActivityFilter && <TextField variant="standard" placeholder="Lọc theo chủ đề hoặc nội dung" sx={filterText}
                            inputProps={{
                    ...
                    <Box sx = {statusContainer}>
                        <Box sx={fillter}>Chưa đọc</Box>
                        <Box sx={fillter}>Đã đọc</Box>
                    </Box>
                </Box>
            ...
            <Box sx={contentContainer}>
                <Routes>
                    <Route path='chat' element={<CurrentChatBox />}/>
